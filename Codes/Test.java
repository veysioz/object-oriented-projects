...
public class Test{
	...
	private static void writeJsonToFile() throws IOException{
		...
		fileWrite.write("\"Dataset Name\": " + "\""+Dataset.datasetName+"\""+",");fileWrite.flush();
		fileWrite.write("\"Maximum Number of Labels per Instance\": " + Dataset.maxNumLabsPerIns+",");fileWrite.flush();
		...
		Dataset.arListLab.forEach((n) -> {
			try {
				if(Dataset.arListLab.size() != counter){
					...
					fileWrite.write("{\"Label ID\": " + n.getLabelID() + ", \"Label Text\": " +"\""+ n.getLabelText()+"\"}");fileWrite.flush();
		...
		Dataset.arListIns.forEach((n) -> {
			try {
				if(Dataset.arListIns.size() != counter){
					fileWrite.write("{\"Instance ID\":" + n.getInstanceID() + ", \"Instance Text\": \"" + n.getInstanceText()+"\"},\n");fileWrite.flush();
				}else{
					fileWrite.write("{\"Instance ID\":" + n.getInstanceID() + ", \"Instance Text\": \"" + n.getInstanceText()+"\"}\n],");fileWrite.flush();
		...
		for(int i = 0; i < Dataset.arListIns.size(); i++) {
			fileWrite.write("{\"Instance ID\": " + Dataset.arListIns.get(i).getInstanceID() + ", \"Class Label IDs\": [");fileWrite.flush();
			counter=1;
			k=i;
			Dataset.arListIns.get(i).getAssignedLabels().forEach((n) -> {
				...
				try {
					...
					if(Dataset.arListIns.get(k).getAssignedLabels().size() !=counter){
						fileWrite.write(n.getLabelID() + " ,");fileWrite.flush();
			...
			fileWrite.write("\"User ID\": " + Dataset.arListIns.get(i).getUser().getUserID()+", ");fileWrite.flush();
			if(i != Dataset.arListIns.size() - 1){
				fileWrite.write("\"Date & Time\": \"" +  formatter.format(Dataset.arListIns.get(i).getDate())+"\"},\n");fileWrite.flush();
			}else{
				fileWrite.write("\"Date & Time\": \"" +  formatter.format(Dataset.arListIns.get(i).getDate())+"\"}\n],");fileWrite.flush();
			}
		}
		...
		for( int i = 0; i < users.size(); i++) {
			if(i == users.size()-1){
				fileWrite.write("{\"User ID\": " + users.get(i).getUserID() + ", \"Username\": \"" + users.get(i).getUserName() + "\", \"User Type\": \"" + users.get(i).getUserType()+"\"}]\n}");
				fileWrite.flush();
			}else{
				fileWrite.write("{\"User ID\": " + users.get(i).getUserID() + ", \"Username\": \"" + users.get(i).getUserName() + "\", \"User Type\": \"" + users.get(i).getUserType()+"\"},\n");
				fileWrite.flush();
